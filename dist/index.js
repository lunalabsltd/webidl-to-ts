...
const makeW3CCompliant = (emscriptenIdl) => {
    ...
    const implementsStatementMatches = [...emscriptenIdl.matchAll(implementsMatcher)];
    const implementsStatementsAsIncludesStatements = content.replace(implementsMatcher, '$1 includes $2;');
    const needsToBeInheritedFrom = [...new Set(implementsStatementMatches.map(([, , implemented]) => implemented))];
...
const roots = parse(compliantSource);
const compile = (webIDLRoots, options) => {
    const host = ts.createCompilerHost(options);
    host.writeFile = (_fileName, contents, _writeByteOrderMark, onError) => {
    ...
    /*customTransformers?: CustomTransformers*/ {
        afterDeclarations: [(context) => (rootNode) => context.factory.updateSourceFile(rootNode, [new CodeGen(context /*, program.getTypeChecker()*/).codegen(webIDLRoots, argv.n)], 
